...
namespace Ilahiyyat.Controllers
{
    ...
    {
        mecidEntities db = new mecidEntities();
        ...
        public ActionResult Index()
        {
            ...
            im._advert = db.Advertisement.Take(3).OrderByDescending(a => a.elanin_başlanqic_günü).ToList();
            im._news = db.News.Take(3).OrderByDescending(n => n.xəberin_əlavə_edilən_gün).ToList();
            im._sermon = db.Sermon.Take(6).ToList();
            im._slider = db.Slider.Take(5).ToList();
            return View(im);
        }
        ...
        public ActionResult Advertisement(int ? page)
        {
            ...
            var _adver = db.Advertisement.OrderByDescending(d => d.elanin_başlanqic_günü).ToList();
            IEnumerable<Advertisement> _adv_list = db.Advertisement.ToList();
        ...
        public ActionResult Question_answer(int ? page)
        {
            ...
            IEnumerable<Questions> _quest_list = db.Questions.ToList();
        ...
        public ActionResult Sermons(int ? page)
        {
            ...
            var _sermons = db.Sermon.ToList();
            IEnumerable<Sermon> _srmn_list = db.Sermon.ToList();
            return View(_sermons.ToPagedList(pageIndex,pageSize));
        }
        public ActionResult Single_advert(int id)
        {
            sam._single_advert = db.Advertisement.Where(v => v.id == id).FirstOrDefault();
            sam._advert = db.Advertisement.Take(5).OrderByDescending(d => d.elanin_başlanqic_günü).ToList();
            return View(sam);
        }
        ...
        {
            spm._pillar = db.Pillar.Where(p => p.id == id).FirstOrDefault();
            spm._pillars = db.Pillar.ToList();
            return View(spm);
        }
        public ActionResult Single_sermon(int id)
        {
            ssm._sermon = db.Sermon.Where(s => s.id == id).FirstOrDefault();
            ssm._sermons = db.Sermon.Take(5).ToList();
            return View(ssm);
        }
        ...
        {
            IEnumerable<Advertisement> adv_list = db.Advertisement.Where(e => e.elanin_adi.Contains(search) || e.elanin_məzmunu.Contains(search)).ToList();
            return View(adv_list);
        }
        public ActionResult BookSearch(string search)
        {
            IEnumerable<Books> book_list = db.Books.Where(e => e.kitabin_adi.Contains(search)).ToList();
            return View(book_list);
        }
        public ActionResult SermonSearch(string search)
        {
            IEnumerable<Sermon> srm_list = db.Sermon.Where(e => e.xutbə_adi.Contains(search) || e.xutbə_məzmunu.Contains(search)).ToList();
