...
namespace Ilahiyyat.Areas.Admin_Panel.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Login(FormCollection frm)
        {
            ...
            var ad = db.Admin.Where(e => e.admin_email == email && e.admin_şifrəsi == password).FirstOrDefault();
            if (ad != null)
            {
                var admin = db.Admin.First(p => p.admin_email == email && p.admin_şifrəsi == password);
        ...
        {
            mm._contact = db.Contact.ToList();
            return View(mm);
        }
        public ActionResult DeleteMessage(int?id)
        {
            Contact cnt = db.Contact.Find(id);
        ...
        public ActionResult QuestSendEmail(string receiver, string subject, string message)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    Questions qst = db.Questions.Find(Session["quest_id"]);
                    qst.cavab = message;
                    db.SaveChanges();
                    return RedirectToAction("Quest");
